...
data_list=load_metadata_from_file('Shared\\Data\\Flight Metadata\\metadata.pkl')
...
def remove_timezone(data_list):
    for entry in data_list:
        if isinstance(entry, dict):
            for key, value in entry.items():
                if isinstance(value, datetime) and value.tzinfo is not None:
                    entry[key] = value.replace(tzinfo=None)
        else:
            print(f"Expected a dictionary, but got {type(entry)}.")
    return data_list
site_list = read_excel_to_list(r'Shared\Data\Inspection data\inspection_data.xlsx')


with open(r'Shared\Data\Flight Metadata\metadata.pkl', 'rb') as f:
    data_list = pickle.load(f)

def main():
    # Load metadata
    flight_data = load_metadata_from_file('Shared\\Data\\Flight Metadata\\updated_metadata.pkl') 
    #passfail_list = load_metadata_from_file('Shared\\Data\\Processed flight data\\passfail_list.pkl')
    # Load site data
    site_list = read_excel_to_list(r'Shared\Data\Inspection data\inspection_data.xlsx')
    passfail_list = []
    # Assuming you have your flight_data and passfail_list
    sorter = FlightSorter(flight_data, passfail_list)
    processed_flight_data, updated_passfail_list = sorter.process_flight_data()

    # Save data
    save_data_to_file(processed_flight_data, 'Shared\\Data\\Flight Metadata\\sortedflightdata.pkl')
    ##print(f"Processed flight data saved to 'Shared\\Data\\Flight Metadata\\flightdata.pkl'.")
    #data_list = remove_timezone(appended_flight_data)
    export_to_excel(processed_flight_data, 'Shared\\Data\\Flight Metadata\\sortedflightdata.xlsx')
    ##print(f"Processed flight data saved to 'Shared\\Data\\Flight Metadata\\flightdata.xlsx'.")
    save_data_to_file(updated_passfail_list, 'Shared\\Data\\Flight Metadata\\sortedpassfail_list.pkl')
    export_to_excel(updated_passfail_list, 'Shared\\Data\\Flight Metadata\\sortedpassfail_list.xlsx')
    ##print(f"Processed pass/fail list saved to 'Shared\\Data\\Flight Metadata\\passfail_list.xlsx'.")
    
    # Save passfail_list
    with open('passfail_list.pkl', 'wb') as f:
        pickle.dump(passfail_list, f)
