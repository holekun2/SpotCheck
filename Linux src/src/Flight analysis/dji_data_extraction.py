...
class SiteLocation(DataImportAndPreprocessing):
    ...
    def load_data(self, metadata_file: str, inspection_data_file: str) -> None:
        # Load metadata from pickle file
        with open(metadata_file, 'rb') as f:
            self.metadata = pickle.load(f)
        
        # Load inspection data from Excel file
        df = pd.read_excel(inspection_data_file)
    ...
    def find_matching_site(self, photo_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # print(f"Finding site for photo: {photo_data.get('File Name', 'Unknown File')}")
        # print(f"Photo coordinates: {photo_lat}, {photo_lon}")
    ...
    def run_analysis(self, metadata_file: str, inspection_data_file: str, output_file: str) -> List[Dict[str, Any]]:
        ...
        self.save_updated_metadata(output_file)
        print(f"Updated metadata has been saved to {output_file}")
        
        return self.metadata
        
        

    def save_updated_metadata(self, output_file: str) -> None:
...
def main():
    # Create an instance of DJIDroneDataImport
    dji_data_import = DJIDroneDataImport()
    
    # Extract metadata
    print("Extracting metadata from photos...")
    appended_metadata = dji_data_import.extract_data(PHOTO_BASE_PATH)
    
    # Save the extracted metadata
    output_file = 'Shared\\Data\\Flight Metadata\\updated_metadata.pkl'
    #print(f"Saving extracted metadata to {output_file}")
    dji_data_import.write_data_to_file(appended_metadata, output_file)
    
    # Also export to Excel for easy viewing
    excel_output = 'Shared\\Data\\Flight Metadata\\updated_metadata.xlsx'
    print(f"Exporting metadata to Excel: {excel_output}")
    export_to_excel(appended_metadata, excel_output)
    
    # Create an instance of SiteLocation
    site_location = SiteLocation()
    
    # Load the extracted metadata and perform site location analysis
    updated_metadata = site_location.run_analysis(
        output_file,
        
        'Shared\\Data\\Inspection data\\inspection_data.xlsx',
        'Shared\\Data\\Processed flight data\\updated_metadata_with_sites.pkl'
    )
    
    # Export the final results to Excel
    final_excel_output = 'Shared\\Data\\Processed flight data\\updated_metadata_with_sites.xlsx'
    print(f"Exporting final results to Excel: {final_excel_output}")
    export_to_excel(updated_metadata, final_excel_output)
    
    # print("Sample of updated metadata:")
    # for entry in updated_metadata[:5]:  # Print first 5 entries as a sample
    #     print(f"File Name: {entry.get('File Name')}")
    #     print(f"Site ID: {entry.get('Site ID')}")
    #     print(f"GPS Coordinates: {entry.get('GPS Latitude')}, {entry.get('GPS Longitude')}")
    #     print("-" * 50)
